...
public class ReportServiceImpl implements ReportService {
    ...
    public OrderReportResponseDTO getOrderStats(OffsetDateTime fromDate, OffsetDateTime toDate) {
        log.info("T\u1ea1o th\u1ed1ng k\u00ea \u0111\u01a1n h\u00e0ng t\u1eeb {} \u0111\u1ebfn {}", fromDate, toDate);
        ...
        BigDecimal avgValue = totalOrders > 0
                ? totalRevenue.divide(BigDecimal.valueOf(completedOrders > 0 ? completedOrders : 1), 2,
        ...
        breakdown.add(StatusBreakdownDTO.builder().status("completed").label("Hoàn thành").count(completedOrders)
                .percentage(totalOrders > 0 ? (completedOrders * 100.0 / totalOrders) : 0).build());
        breakdown.add(StatusBreakdownDTO.builder().status("pending").label("Chờ xử lý").count(pendingOrders)
                .percentage(totalOrders > 0 ? (pendingOrders * 100.0 / totalOrders) : 0).build());
        ...
        breakdown.add(StatusBreakdownDTO.builder().status("processing").label("Đang xử lý")
                .count(Math.max(0, processingOrders))
                .percentage(totalOrders > 0 ? (Math.max(0, processingOrders) * 100.0 / totalOrders) : 0).build());
        breakdown.add(StatusBreakdownDTO.builder().status("cancelled").label("Đã hủy").count(cancelledOrders)
                .percentage(totalOrders > 0 ? (cancelledOrders * 100.0 / totalOrders) : 0).build());
        response.setStatusBreakdown(breakdown);

        // Recent Orders
        List<com.eaut.backend.entities.Order> recentEntities = orderRepository.findRecentOrdersByDateRange(fromDate,
                toDate, org.springframework.data.domain.PageRequest.of(0, 5));
    ...
    public ProductReportResponseDTO getProductStats(OffsetDateTime fromDate, OffsetDateTime toDate) {
        log.info("T\u1ea1o th\u1ed1ng k\u00ea s\u1ea3n ph\u1ea9m t\u1eeb {} \u0111\u1ebfn {}", fromDate, toDate);
        ...
        long outOfStockProducts = productRepository
                .countProductsByStatus(com.eaut.backend.constant.ProductStatus.inactive); // Or customized logic
        ...
        // Chart Data
        List<java.util.Map<String, Object>> dailyStats = orderItemRepository.getDailySoldProductsStats(fromDate,
                toDate);
        ...
        for (java.util.Map<String, Object> stat : dailyStats) {
            ...
            chartData.add(
                    com.eaut.backend.model.response.report.ChartDataDTO.builder().date(dateStr).sold(sold).build());
        }
        response.setChartData(chartData);

        // Top Selling
        List<java.util.Map<String, Object>> topSellingList = orderItemRepository.getTopSellingProducts(fromDate, toDate,
    ...
    public ChatbotReportResponseDTO getChatbotStats(OffsetDateTime fromDate, OffsetDateTime toDate) {
        log.info("T\u1ea1o th\u1ed1ng k\u00ea chatbot t\u1eeb {} \u0111\u1ebfn {}", fromDate, toDate);
        ...
        // Mocking average response time and satisfaction rate for now as it requires
        // complex parsing or AI analysis over message timestamps
        ...
        List<java.util.Map<String, Object>> dailyConvStats = conversationRepository.getDailyConversationStats(fromDate,
                toDate);
        List<java.util.Map<String, Object>> dailyMsgStats = chatMessageRepository.getDailyMessageStats(fromDate,
                toDate);
        ...
        for (java.util.Map<String, Object> stat : dailyConvStats) {
            ...
            chartDataMap.put(dateStr,
                    ChartDataDTO.builder().date(dateStr).conversations(convCount).messages(0L).build());
        }

        for (java.util.Map<String, Object> stat : dailyMsgStats) {
            String dateStr = String.valueOf(stat.get("date"));
            Long msgCount = ((Number) stat.get("messages")).longValue();
            ChartDataDTO dto = chartDataMap.getOrDefault(dateStr,
                    ChartDataDTO.builder().date(dateStr).conversations(0L).messages(0L).build());
        ...
        topQuestions.add(com.eaut.backend.model.response.report.TopQuestionDTO.builder()
                .question("Giá iPhone 15 Pro Max bao nhiêu?").count(125).build());
        topQuestions.add(com.eaut.backend.model.response.report.TopQuestionDTO.builder()
                .question("Có hỗ trợ trả góp không?").count(98).build());
        topQuestions.add(com.eaut.backend.model.response.report.TopQuestionDTO.builder()
                .question("Thời gian bảo hành bao lâu?").count(85).build());
        response.setTopQuestions(topQuestions);

        // Recent Conversations
        List<com.eaut.backend.entities.Conversation> recentEntities = conversationRepository
                .findRecentConversationsByDateRange(fromDate, toDate,
                        org.springframework.data.domain.PageRequest.of(0, 5));
        ...
        for (com.eaut.backend.entities.Conversation c : recentEntities) {
            recentConversations.add(com.eaut.backend.model.response.report.RecentConversationDTO.builder()
                    ...
                    .date(c.getCreatedAt().toLocalDate().toString() + " "
                            + String.format("%02d:%02d", c.getCreatedAt().getHour(), c.getCreatedAt().getMinute()))
                    .build());
        }
