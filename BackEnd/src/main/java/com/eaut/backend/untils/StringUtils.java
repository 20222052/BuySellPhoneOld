...
public class StringUtils {
    public static void validatePassword(String password) throws ApplicationException {
        ...
        if (org.apache.commons.lang3.StringUtils.isEmpty(password) || password.length() < 8) {
            throw new ApplicationException(ErrorCode.INVALID_PARAMETER, "mật khẩu không hợp lệ");
        }

        if (!password.matches(".*[a-zA-Z].*")) {
            throw new ApplicationException(ErrorCode.INVALID_PARAMETER, "mật khẩu phải chứa ít nhất một chữ cái");
        }
    }
}
